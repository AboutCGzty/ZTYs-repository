...
​	（1）`float4 vertex : POSITION;`：代表获取模型本地空间下的顶点坐标位置信息，顶点坐标位置是一个四维浮点数值。

​	（2）`float3 normal : NORMAL;`：代表获取模型本地空间下的顶点法线信息，顶点法线是一个三维浮点数值。

​	（3）`float2 uv : TEXCOORD0;`：代表获取模型切线空间下的顶点 UV 坐标信息，顶点 UV 坐标是一个二维浮点数值。
...
​	（1）`float4 pos_clip : SV_POSITION;`：代表需要转换且输出的裁剪空间下的顶点坐标位置信息，是一个四维浮点数值。

​	（2）`float2 uv_mesh : TEXCOORD0;`：代表需要转换且输出的切线空间下的顶点 UV 坐标信息，是一个二维浮点数值。

​	（3）`float3 pos_world : TEXCOORD1;`：代表需要转换且输出的世界空间下的顶点坐标位置信息，是一个三维浮点数值。

​	（4）`float3 normal_world : TEXCOORD2;`：代表需要转换且输出的世界空间下的顶点法线信息，是一个三维浮点数值。
...
​	如上示例代码可以看到，“顶点”着色器是一个类型为“VertexOutput”，名为“vert”的方法函数。其输入参数为“VertexInput”，并命名为“v（代表vertex）”，刚好与上述的所有部分名称所对应。

​	其中：

​	（1）`VertexOutput o;`：代表对“顶点”着色器的初始化。
